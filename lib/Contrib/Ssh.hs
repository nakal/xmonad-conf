...
--
-- >   , ((modm .|. controlMask, xK_s), sshPrompt def (\p -> runInTerm "" $ "ssh " ++ p))
-- or:
-- >   , ((modm .|. controlMask, xK_s), sshPrompt def (\p -> runInTerm "" $ "ssh -t " ++ p ++ " tmux new-session"))
--
...
sshPrompt :: XPConfig           -- ^ prompt configuration
        -> (String -> X ())     -- ^ function to execute with the prompt input
        -> X ()
sshPrompt c cmd = do
  sc <- io sshComplList
  mkXPrompt Ssh c { historyFilter = deleteAllDuplicates } (mkComplFunFromList c sc) cmd
