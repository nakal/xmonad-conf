...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((modm,			xK_v     ), spawn $ (XMonad.terminal conf) ++ " -e tmux -2 new-session 'vim -c \"Unite -toggle -start-insert file_rec/async\"'")
    , ((modm .|. shiftMask,	xK_v     ), spawn $ (XMonad.terminal conf) ++ " -e vim ~/.vim/vimrc")
    , ((modm .|. controlMask,	xK_v     ), spawn $ "cd ~/src/nid-prototype ; " ++ (XMonad.terminal conf) ++ " -e tmux -2 new-session 'vim -c \"Unite -toggle -start-insert file_rec/async\"'")
